...
record_new_crash_files() {
  ...
  local mono_record
  mono_record="$(latest_file_epoch "${VERIFY_APP_PATH}/Contents" 'mono_crash.*.json' || true)"
  if [[ -z "${mono_record}" ]]; then
    mono_record="$(latest_file_epoch /tmp 'mono_crash.*.json' || true)"
  fi
  if [[ -n "${mono_record}" ]]; then
  ...
  diagnostic_record="$(
    {
      latest_file_epoch "${HOME}/Library/Logs/DiagnosticReports" 'TokenForge*.crash' || true
      latest_file_epoch "${HOME}/Library/Logs/DiagnosticReports" 'TokenForge*.ips' || true
    } | sort -nr | head -1
  )"
...
section "Preflight"
PREFLIGHT_STATUS=0
RUN_TOKENFORGE_OPEN_PROBE=false REQUIRE_APP_INTEGRITY=false APP_BUNDLE_PATH="${BUILD_OUTPUT}" "${SCRIPT_DIR}/preflight-runtime-environment.sh" || PREFLIGHT_STATUS=$?
...
if record_new_crash_files_with_wait "${launch_epoch}"; then
  ...
  if LAUNCHSERVICES_ABORT_CRASH="$(launchservices_registration_abort_detected)"; then
